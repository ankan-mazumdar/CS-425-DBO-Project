...
def custom_query():
    ...
    window_query.geometry("400x400")  # Set the size of the window

    tk.Label(window_query, text="Enter SQL Query (semicolon-separated for multiple queries):", font=("Helvetica", 14)).pack()
    query_entry = tk.Text(window_query, width=50, height=8, font=("Helvetica", 10))
    query_entry.pack()

    result_text = tk.Text(window_query, width=50, height=30)
    result_text.pack()
    ...
    tk.Button(window_query, text="Execute Query", command=execute_query, font=("Helvetica", 10)).pack()
